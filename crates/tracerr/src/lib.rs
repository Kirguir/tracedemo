...
/// let err: Traced<u64> =
///     res.map_err(tracerr::from_and_wrap!()).unwrap_err();
/// ```
#[macro_export]
...
mod map_from_and_wrap_macro_spec {
    ...
    fn applies_required_from_implementation() {
        ...
        let res = res.map_err(super::wrap!());
        let err: Traced<u64> = res.map_err(super::map_from_and_wrap!()).unwrap_err();
        assert!(!err.trace.is_empty(), "captures frames");
    }
}
...
mod from_and_wrap_macro_spec {
    ...
    fn applies_required_from_implementation() {
        let res: Result<(), u8> = Err(54);
        let err: Traced<u64> = res.map_err(super::from_and_wrap!()).unwrap_err();
        assert!(!err.trace.is_empty(), "captures frames");
    }
}
